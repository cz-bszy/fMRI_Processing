...
NOTES = {
    ...
    "denoise": "每种去噪策略的定量结果。DOF left = 保留帧数 − 回归量个数 − 带通滤波消耗的自由度，低于 MIN_DOF 标为 warn；"
               ...
               "FD-DVARS r (post)：正值说明头动造成的信号跳变仍在（去噪不足）。负值说明高 FD 帧的残差比安静帧还小："
               "头动扩展回归量（以及与头动相关的 aCompCor 成分）在这些帧取极值、杠杆高，回归几乎把这些帧拟合掉，"
               "相当于软删帧（这些帧的神经信号也一起去掉）；带通使剩余自由度很少时会放大这一效应。"
               "短 run 用 24 参数模型时常见 −0.2 ~ −0.45，本身不是失败；同样自由度的随机回归量不会产生负值。"
               "若同时 DOF 很低，优先靠 censoring 处理高运动帧，或改 FILTER_MODE=highpass 保留自由度。"
               "variance removed 很高而 DOF 很低时要警惕过度回归。",
